...
axios.interceptors.response.use(
  async (response) => {
    console.log(`Done ${response.config.method?.toUpperCase()}: ${response.config.url}`)

    if (response.status === 200 && (response.data.message || response.data.error_message))
  ...
  async (error) => {
    ...
    switch (Number(error.response?.status)) {
      ...
      case 403:
        ...
        emitter.emit('showSnackbar', { text: 'Доступ к запрошенному ресурсу запрещен' })
        break
      default: {
